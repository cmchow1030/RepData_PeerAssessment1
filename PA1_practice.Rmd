...
```{r interval with the maximum number of steps}
...
```{r missing values}
no_NA <- nrow(data[is.na(data$steps), ])
...
```{r mean total number of steps with the new dataset}
#calculating total steps taken on a day
...
interval_step2 <- tapply(data_noNA$steps,
                        ...
                        function(x){mean(x, na.rm = TRUE)})
#converting the resulted matrix into a data frame for checking
df_is2 <- data.frame(
         interval = as.numeric(colnames(interval_step2)),
         steps_weekday = as.numeric(interval_step2[1,]),
         steps_weekend = as.numeric(interval_step2[2,])
 )
#plotting a line graph using base R
plot(x = df_is2$interval, 
     y = df_is2$steps_weekday, 
     type = "l",
     main = "Average Daily Activity Pattern",
     xlab = "Interval (min)",
     ylab = "Average Number of Steps",
     col = "blue")
# Add weekend line 
lines(x = df_is2$interval, y = df_is2$steps_weekend, col = "red", lwd = 2) 
# Add legend 
